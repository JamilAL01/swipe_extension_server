...
function showConsentPopup() {
  ...
  document.getElementById("lang-select").onchange = (e) => {
    selectedLang = e.target.value;
    localStorage.setItem("swipeLang", selectedLang);
    popup.remove(); // remove old popup first
    showConsentPopup(); // render new popup in new language
  };
  ...
  document.getElementById("consent-yes").onclick = () => {
    ...
    console.log("[SwipeExtension] User consented ✅");
    initExtension(true);       // start tracking
    setTimeout(showSurveyPopup, 50); // ensure survey renders after DOM ready
  };
